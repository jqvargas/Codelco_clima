...
for(i in 6:ncol(pp)){
  for(j in 1:nrow(pp)){
    if(is.na(pp[j,i])){
      pp[j,i] <- median(pp[which(pp$mes==pp$mes[j]),i], na.rm=T)
    }
  }
}
...
for(i in 6:ncol(tn)){
  for(j in 1:nrow(tn)){
    if(is.na(tn[j,i])){
      tn[j,i] <- mean(tn[which(tn$mes==tn$mes[j]),i], na.rm=T)
    }
  }
}
for(i in 6:ncol(tx)){
  for(j in 1:nrow(tx)){
    if(is.na(tx[j,i])){
      tx[j,i] <- mean(tx[which(tx$mes==tx$mes[j]),i], na.rm=T)
    }
  }
}
...
if (!is.null(q)) {
  ...
  for(i in 6:ncol(q)){
    for(j in 1:nrow(q)){
      if(is.na(q[j,i])){
        q[j,i] <- mean(q[which(q$mes==q$mes[j]),i], na.rm=T)
      }
    }
  }
...
for(j in 1:(ncol(tx.m)-2)){
  ...
  res0 <- data.frame(var = "Tx", estacion=colnames(tn)[j+5], mes="Anual",pend_decada= pend, p_valor=mkp)
  for(i in 1:12){
...
for(i in 6:ncol(pp)){
  ...
  r75.season <- aggregate(pp[,i], list(pp[,4], pp[,5]), function(x){length(which(x > v.p95))})
